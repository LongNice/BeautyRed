	...
	<title>BeautyRed</title>
		...
		<script type="text/javascript">
			...
			function DynamicPeopleTable(){
				var IDArray = ["<?php echo join("\",\"",$IDArray);?>"];
				var ImgArray = ["<?php echo join("\",\"",$ImgArray);?>"]; //PHP_NameArray轉JS_NameArray
				...
				for(var people = 0; people < <?php echo $People_number;?>; people++){
					...
					//***取得圖片資料***
					var Img = new Image();
						Img.src = 'uploads/'+ImgArray[people];
					...
					PeopleDiv.onclick = function(){ //轉換到遊戲頁面
							window.location.assign("http://longnice.clouds.twgogo.org/Game.php?PeopleID="+this.id); //row_num+=4:name department FB Img
							};
